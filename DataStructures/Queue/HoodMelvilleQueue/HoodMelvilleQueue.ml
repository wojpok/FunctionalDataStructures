...
module DiffedHoodMelvilleQueue = struct
  ...
  (*              v---- := lenf - lenr *)
  type 'a queue = int * 'a list * 'a rotating_state * 'a list
  ...
  let empty = (0, [], Idle, [])
  let isEmpty (_, xs, _, _, _) = List.is_empty xs

  let snoc x (d, f, st, r) = check (d - 1, f, st, x :: r)
